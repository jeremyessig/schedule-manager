...
func _update_gui() ->void:
	...
	var duration :Array= time.get_time_24h_StringArray(lesson_card.schedule[2])
	...
	schedule_days_option_button.select(get_item_index_by_string(schedule_days_option_button, lesson_card.schedule[0]))
	var schedule = time.get_time_00h00_StringArray(lesson_card.schedule[1])
...
func _on_EditButton_pressed():
	...
	if (Global.get_item_string(type_option_button) != lesson_card.type or
		...
		Global.get_item_string(schedule_days_option_button) != lesson_card.schedule[0] or
